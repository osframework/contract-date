/*
 * File: AbstractDelimitedTextFileInfoOutput.java
 * 
...
import org.osframework.contract.date.fincal.data.AbstractInfoOutput;
import org.osframework.contract.date.fincal.model.FinancialCalendar;

/**
 * AbstractDelimitedTextFileInfoOutput description here.
 *
 * @author <a href="mailto:dave@osframework.org">Dave Joyce</a>
 */
public abstract class AbstractDelimitedTextFileInfoOutput extends AbstractInfoOutput<OutputStream, IOException> {
	...
	public AbstractDelimitedTextFileInfoOutput(final OutputStream out, final String delimiter) throws IOException {
		super();
	...
	@Override
	public void store(FinancialCalendar... calendars) throws IOException {
		try {
			synchronized (lock) {
				if (0 == recordCount) {
				...
				for (int i = 0; i < calendars.length; i++) {
					...
					writer.write(calendarToRecord(calendars[i]));
				}
			}
	...
	protected abstract String calendarToRecord(FinancialCalendar calendar);
